...
async function runDeployment() {
  ...
  try {
    ...
    // 2. Build containers (using docker compose v2 plugin)
    console.log('[DEPLOY] Building containers...');
    await execPromise(`docker compose -f ${REPO_PATH}/admin-platform/docker-compose.ufitech.yml build`);

    // 3. Restart containers with zero downtime
    console.log('[DEPLOY] Restarting containers...');
    await execPromise(`docker compose -f ${REPO_PATH}/admin-platform/docker-compose.ufitech.yml up -d`);
